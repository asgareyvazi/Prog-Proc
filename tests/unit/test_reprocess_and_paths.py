...
def test_reprocess_uses_the_hash_of_the_file_on_disk(workspace, corpus, registered, ids) -> None:
    ...
    target.write_text(target.read_text(encoding="utf-8") + "Revised after the pressure test: EMW 11.1 ppg.\n", encoding="utf-8")
    ...
    with workspace.database.session() as session:
        ...
        assert version.supersedes_version_id == old.id and old.superseded_by_version_id == version.id
...
def test_reprocess_of_a_missing_source_is_an_error_and_changes_nothing(workspace, corpus, registered) -> None:
    ...
    assert (versions_after, hash_after) == (versions_before, hash_before), "a failed reprocess must be a no-op"
...
def test_version_records_a_workspace_relative_reference(workspace, corpus, registered) -> None:
    with workspace.database.read_only() as session:
        ...
        assert not Path(version.source_relative_path).is_absolute(), "the durable reference is relative by definition"
...
def test_provenance_still_verifies_after_the_workspace_moves(workspace, corpus, registered, tmp_path) -> None:
    ...
    with workspace.database.read_only() as session:
        ...
        resolved = repository.resolve_source_path(moved_version, moved_document, workspace_root=workspace.root)
        assert resolved is None, "the original really is gone"
        resolved_after_move = repository.resolve_source_path(moved_version, moved_document, workspace_root=relocated)
...
def test_candidate_paths_prefer_the_recorded_absolute_then_the_relative(workspace, tmp_path) -> None:
    ...
    assert [str(item) for item in candidates] == [str(absolute), str(tmp_path / "ws" / "docs/doc.pdf")]
    ...
    fallback = candidate_source_paths(recorded_path="", workspace_root=tmp_path / "ws", relative_path="", filename="doc.pdf")
...
def test_timestamps_are_honest_about_creation() -> None:
    ...
    if not has_creation_time_support():
        assert timestamps.created_at is None, "an inode change time must not be passed off as a creation time"
...
def test_registry_stores_creation_only_when_the_platform_has_one(workspace, corpus, registered) -> None:
    with workspace.database.read_only() as session:
        document = session.get(Document, registered.document_id)
        ...
        if has_creation_time_support():
            ...
            assert document.file_created_at is None, "Linux has no birth time here; NULL is the honest answer"
