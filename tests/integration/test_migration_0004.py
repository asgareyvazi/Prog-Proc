...
def test_the_upgrade_creates_every_table_the_models_declare(tmp_path) -> None:
    ...
    try:
        ...
        # "The migrations describe the models" is a claim about the *head* of the chain, not about 0004:
        # a migration added later widens a legacy table, and comparing at 0004 would report that as a
        # defect in the schema rather than as the gap this test is not trying to close.
        forward = upgrade(engine, heads()[0])
        assert forward.current == heads()[0] and forward.mode == "migrated", forward.to_dict()
...
def test_the_downgrade_removes_the_domain_and_keeps_the_workspace(tmp_path) -> None:
    ...
    try:
        ...
        assert snapshot(engine) == before
        # Back at head, so the file has to be indistinguishable from a database the models built.
        assert schema_diff(engine) == {
            "missing_tables": [],
            "extra_tables": [],
            "missing_columns": [],
