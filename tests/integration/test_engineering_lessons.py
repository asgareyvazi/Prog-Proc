...
# --------------------------------------------------------------------------- engineering records
#
# ``calculation`` and ``calculation_input`` have been in the schema since 0001 with a read path and no
# write path, which made "what backs this number" a question with a table for an answer and no way to put
# anything in it.  These tests are the write path: persistence only, no arithmetic, and the same three
# promises every other record makes - a method is named, evidence is required from anything not typed by
# hand, and re-running a batch is not a second copy of the same claim.


def _annular(repository, hierarchy, **overrides):
    values = {
        "method_id": "hydraulics.annular_pressure",
        "method_version": "3.1",
        "calculation_type": "hydraulics",
        "well_id": hierarchy["well_a"].id,
        "section_id": hierarchy["section"].id,
        "inputs": {
            "mud_weight": {
                "value": "10.2 ppg",
                "unit": "ppg",
                "subject_key": "well:A-3|mud_weight",
            },
            "flow_rate": {"value": 480.0, "unit": "gpm", "subject_key": "program:ncf-01|flow_rate"},
            "hole_annulus": 0.082,
        },
        "outputs": {"ecd_ppg": 11.4, "apv_psi": 312.0},
        "assumptions": ["rheology from the Fann 6-speed, read the same morning"],
        "validation": {"checked_against": "survey 2025-06-12", "within_tolerance": True},
        "confidence": 0.8,
        "triggered_by": "cli",
    }
...
def test_an_engineering_record_keeps_its_method_and_its_numbers(session, hierarchy) -> None:
    repository = EngineeringRepository(session)
    row, created = _annular(repository, hierarchy)
    session.commit()
    assert created is True
    assert row.method_id == "hydraulics.annular_pressure" and row.method_version == "3.1"
    assert row.well_id == hierarchy["well_a"].id and row.section_id == hierarchy["section"].id
    assert row.outputs == {"ecd_ppg": 11.4, "apv_psi": 312.0}
    assert row.assumptions == ["rheology from the Fann 6-speed, read the same morning"]
    assert row.validation["within_tolerance"] is True
    # A hand-entered record is MANUAL and cites nothing; the platform does not claim a source it has not seen.
    assert row.origin == "MANUAL" and row.created_by == "system"
    assert row.provenance == [] and row.document_version_id is None
    assert row.status == "COMPUTED" and row.revision == 1
    assert row.identity_key and row.identity_key.startswith("calc:")
    assert row.attributes == {}

    [listed] = repository.calculations_for(well_id=hierarchy["well_a"].id)
    assert listed.id == row.id
    # The units came from the payload, not from a column, and a bare number stays a bare number.
    inputs = repository.calculation_inputs(row.id)
    by_name = {item.name: item for item in inputs}
    assert by_name["mud_weight"].value == pytest.approx(10.2)
    assert by_name["mud_weight"].unit == "ppg"
    assert by_name["flow_rate"].value == pytest.approx(480.0)
    assert by_name["hole_annulus"].value == pytest.approx(0.082)
    assert by_name["hole_annulus"].source_kind == "user"
    assert by_name["mud_weight"].source_kind == "knowledge"
    # The unit registry, not the caller, fills the dimension - and a bare number stays dimensionless
    # rather than being assigned a dimension it never claimed.
    assert by_name["mud_weight"].dimension == "MUD_WEIGHT"
    assert by_name["flow_rate"].dimension == "FLOW_RATE"
    assert by_name["hole_annulus"].unit == "" and by_name["hole_annulus"].dimension == ""


def test_recording_the_same_claim_twice_is_a_no_op(session, hierarchy) -> None:
    """A batch that runs twice must not double-count an engineering check.

    Identity is content, so the second call returns the row it wrote; different content is a *new* row,
    because the older one may still be what a decision was made on.  The database's unique index on
    ``identity_key`` is what makes that hold when two callers write at the same moment, so it is asserted
    directly too.
    """
    repository = EngineeringRepository(session)
    first, created = _annular(repository, hierarchy)
    session.commit()
    again, created_again = _annular(repository, hierarchy)
    session.commit()
    assert created is True and created_again is False
    assert again.id == first.id
    assert len(repository.calculations_for(well_id=hierarchy["well_a"].id)) == 1
    assert len(repository.calculation_inputs(first.id)) == 3, "the input index was appended to"

    changed, created_changed = _annular(
        repository, hierarchy, outputs={"ecd_ppg": 11.6, "apv_psi": 320.0}
    )
    session.commit()
    assert created_changed is True and changed.id != first.id
    assert len(repository.calculation_inputs(changed.id)) == 3
    assert {row.id for row in repository.calculations_for(well_id=hierarchy["well_a"].id)} == {
        first.id,
        changed.id,
    }
    ...
    from drilling_intelligence.database.models import Calculation
...
def test_a_record_that_claims_a_source_has_to_name_one(session, hierarchy) -> None:
    """The provenance invariant, enforced by storage rather than by a caller remembering it."""
    repository = EngineeringRepository(session)
    with pytest.raises(ValidationError, match="has to cite its evidence"):
        _annular(repository, hierarchy, origin="EXTRACTED")
    with pytest.raises(ValidationError, match="has to name its method"):
        repository.record_calculation(method_id="   ")

    cited, created = _annular(
        repository,
        hierarchy,
        origin="DERIVED",
        provenance=[{"kind": "cell", "sheet": "Summary", "ref": "Summary!B9"}],
        created_by="promoter",
    )
    session.commit()
    assert created is True
    assert cited.origin == "DERIVED" and cited.created_by == "promoter"
    assert cited.provenance[0]["ref"] == "Summary!B9"


def test_an_engineering_record_can_be_superseded_but_never_rewritten(session, hierarchy) -> None:
    repository = EngineeringRepository(session)
    old, _ = _annular(repository, hierarchy)
    session.commit()
    new, created = _annular(
        repository,
        hierarchy,
        outputs={"ecd_ppg": 11.1, "apv_psi": 290.0},
        supersedes_id=old.id,
        reviewer="p.ndiaye",
        approval_note="re-run after the bit change",
    )
    session.commit()
    assert created is True
    assert new.supersedes_id == old.id
    assert new.revision == old.revision + 1, (
        "the chain counts up, and the numbers are the row's own"
    )
    assert str(old.status) == "SUPERSEDED" and str(new.status) == "COMPUTED"
    assert new.reviewer == "p.ndiaye" and new.approval_note == "re-run after the bit change"
    # The superseded record is still readable, which is the point of a chain rather than a column.
    assert old.id in {row.id for row in repository.calculations_for(well_id=hierarchy["well_a"].id)}
    assert repository.calculations_for(well_id=hierarchy["well_a"].id, current_only=True) == [new]
    with pytest.raises(ValidationError, match="no such engineering record"):
        repository.get_calculation("calc-nope")


def test_a_unit_inside_the_value_is_not_lost(session, hierarchy) -> None:
    """``{"value": "10.2 ppg"}`` with no `unit` key still indexes as 10.2 ppg, dimension MUD_WEIGHT.

    A payload written by a spreadsheet bridge tends to put the unit in the string, because that is what
    the cell holds.  Reading the number and discarding the unit would leave a value nobody can compare.
    """
    repository = EngineeringRepository(session)
    row, _ = repository.record_calculation(
        method_id="hydraulics.hydrostatic",
        inputs={"mw": "10.2 ppg", "tvd": "9850 ft", "grip": "unparseable"},
    )
    session.commit()
    by_name = {item.name: item for item in repository.calculation_inputs(row.id)}
    assert by_name["mw"].value == pytest.approx(10.2) and by_name["mw"].unit == "ppg"
    assert by_name["mw"].dimension == "MUD_WEIGHT"
    assert by_name["tvd"].value == pytest.approx(9850.0) and by_name["tvd"].unit == "ft"
    assert by_name["tvd"].dimension == "LENGTH"
    # Something that is not a number at all is still stored, with no value and no invented dimension.
    assert by_name["grip"].value is None and by_name["grip"].dimension == ""
    assert by_name["grip"].unit == ""


def test_inputs_are_indexed_for_change_impact(session, hierarchy) -> None:
    """ "Which numbers used this value?" is a query, not a scan of every payload.

    The index exists so that a correction to a mud weight - or the retirement of the document it came from
    - can be followed through to the engineering results that consumed it.  A record whose inputs name no
    subject is still stored, it simply cannot be found this way: nothing is invented to make the list
    longer.
    """
    repository = EngineeringRepository(session)
    row, _ = _annular(repository, hierarchy)
    plain, _ = repository.record_calculation(
        method_id="torque.drag_overview",
        calculation_type="torque_drag",
        inputs={"top_drive": 240.0},
    )
    session.commit()
    using_mud = repository.calculations_using("well:A-3|mud_weight")
    assert [item.id for item in using_mud] == [row.id]
    assert [item.id for item in repository.calculations_using("program:ncf-01|flow_rate")] == [
        row.id
    ]
    assert repository.calculations_using("nothing:mentions|this") == []
    assert plain.identity_key != row.identity_key
    with pytest.raises(ValidationError, match="no input subject"):
