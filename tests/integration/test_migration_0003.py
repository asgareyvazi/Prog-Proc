...
def test_the_downgrade_is_the_exact_inverse(tmp_path) -> None:
    ...
    try:
        ...
        # 0002 is behind the head of the chain by more than 0003's own columns, and the diff says so.
        # This test is about the inverse being exact for the migration under test, so it looks at those
        # columns - nothing else may be missing from knowledge_item.
        assert sorted(
            item for item in diff["missing_columns"] if item.startswith("knowledge_item.")
        ) == sorted(f"knowledge_item.{column}" for column in ADDED_COLUMNS), diff
        assert not [item for item in diff["extra_columns"] if item.startswith("knowledge_item.")], (
            diff
        )
