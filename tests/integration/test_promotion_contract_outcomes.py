...
def test_forensic_batch_reports_evidence_only_files_without_writing_them(workspace) -> None:
    ...
    # The generated corpus has six current extracted versions: three certified/promotable domain
    # sources and three evidence-only/degraded sources.  The latter are visited only because this is
    # the explicit forensic mode.
    assert summary["versions"] == 6, summary
    assert summary["outcomes"]["unsupported"] >= 3, summary
    assert summary["eligibility"]["ineligible"] >= 3, summary
    assert summary["skipped"].get("UNSUPPORTED_CLASSIFICATION", 0) >= 3, summary
