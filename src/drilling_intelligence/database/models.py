...
class Calculation(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_calculation_well", "well_id", "method_id"),
        # Content identity, the same convention the operational and engineering records use: a batch
        # that runs twice returns the row it wrote instead of appending a twin of it.  A unique *index*
    ...
    error: Mapped[str | None] = mapped_column(Text)
    # The evidence convention this table arrived at in 0001 without (ADR-0009, widened by 0005): where a
    # number came from, who or what wrote it, and the content identity that makes a re-run a no-op.  A
    # calculation read out of a report is EXTRACTED and cites the version; one a person typed is MANUAL
    # and cites nothing - and the repository refuses the first without the second.
    origin: Mapped[str] = mapped_column(String(16), default="MANUAL", nullable=False)
    created_by: Mapped[str] = mapped_column(String(80), default="system", nullable=False)
    identity_key: Mapped[str | None] = mapped_column(String(160))
    # No foreign key here, and the reason is in migration 0005's docstring: SQLite cannot add one to an
    # existing table without a rebuild that ``--sql`` cannot express, and a constraint only some workspaces
    # have is worse than a check every workspace passes.  ``check_promoted_evidence`` is what polices them.
