    ...
    *,
    ...
    if record_type == "procedure_record":
        ...
        if "APPROVED" in targets and str(data.get("rationale") or "").strip():
            actions.append(
                _action("approve", "Approve", "APPROVED", independent=True)
            )
...
class ReviewActionService:
    ...
    def execute(self, request: ReviewActionRequest | Mapping[str, Any]) -> ReviewActionResult:
        ...
        req = request if isinstance(request, ReviewActionRequest) else ReviewActionRequest(**dict(request))
    ...
    def _execute_conflict(self, session: Session, req: ReviewActionRequest) -> ReviewActionResult:
        ...
        if req.action != "resolve_conflict":
            raise ReviewActionError(f"action {req.action!r} is not available for a knowledge conflict")
    ...
    def _find_capability(self, session: Session, row: Any, req: ReviewActionRequest) -> ReviewAction:
    ...
    def _validate_input(req: ReviewActionRequest, capability: ReviewAction) -> None:
        if capability.requires_actor and not req.actor:
            raise ReviewActionError("a human action needs an explicit actor", hint="enter your name or user id")
    ...
    def _route(
        ...
        if record_type == "procedure_record":
            ...
            if req.action == "approve":
                ...
                repository.set_procedure_status(str(row.id), target, by=req.actor, reason=explanation)
            return
        if record_type == "drilling_program":
            repository = EngineeringRepository(session)
            if req.action == "approve":
        ...
        if record_type == "cost_item":
            ...
            return
        if record_type in _OPERATIONAL_MODELS:
            OperationsRepository(session).set_status(
                row, target, by=req.actor, reason=explanation
            )
            return
