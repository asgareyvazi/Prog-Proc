...
@dataclass(frozen=True)
class NptRollupResult:
    """Plain result returned across the UI worker boundary after an explicit NPT execution."""
...
class ReviewController:
    ...
    def run_npt_rollup(
        ...
        *,
        ...
        """Execute the registered NPT V1 method through the application-service boundary.

        The controller owns no arithmetic and returns a plain result so a worker never hands an ORM
        object to a widget.  This is the only UI path that executes a calculation; review loading
        remains read-only and never calls it.
        """
        workspace = self._require_workspace()
        actor = str(created_by or "").strip()
        if not actor:
            raise ValidationError(
                "an NPT roll-up needs an explicit actor", hint="enter your name or user id"
            )
        calculation, created = EngineeringService.for_workspace(workspace).record_npt_rollup(
            well_id=str(well_id),
            created_by=actor,
            supersedes_id=str(supersedes_id or ""),
        )
        metric = (calculation.outputs or {}).get("npt_hours") or {}
        return NptRollupResult(
            calculation_id=str(calculation.id),
            created=bool(created),
            method_id=str(calculation.method_id),
            method_version=str(calculation.method_version),
            well_id=str(calculation.well_id or ""),
            value=metric.get("value"),
            unit=str(metric.get("unit") or ""),
            validation=dict(calculation.validation or {}),
            revision=int(calculation.revision or 1),
            supersedes_id=str(calculation.supersedes_id or ""),
            created_by=str(calculation.created_by or ""),
        )

    def available_actions(self, record: ReviewRecord | ReviewConflict) -> tuple[ReviewAction, ...]:
    ...
    def execute_action(self, request: ReviewActionRequest | dict[str, Any]) -> ReviewActionResult:
...
__all__ = ["NptRollupResult", "ReviewController", "WellChoice"]
